...
import {
  getCollaboratorCategoryRankListSuccess,
  getCollaboratorCategoryRankListError,
} from './actions';
...
function* getCollaboratorCategoryRankListByCategory(action) {
  try {
    const { data: ranks } = yield call(
      api.categories.collaboratorRanks,
      action.categoryId,
      action.periodId,
    );
...
function* getCollaboratorCategoryRankListByCollaborator(action) {
  try {
    const { data: ranks } = yield call(
      api.collaborators.categoryRanks,
...
export function* watchCollaboratorCategoryRankListByCategory() {
  yield takeLatest(
    types.GET_COLLABORATOR_CATEGORY_RANK_LIST_BY_CATEGORY,
    getCollaboratorCategoryRankListByCategory,
  );
}

export function* watchCollaboratorCategoryRankListByCollaborator() {
  yield takeLatest(
    types.GET_COLLABORATOR_CATEGORY_RANK_LIST_BY_COLLABORATOR,
    getCollaboratorCategoryRankListByCollaborator,
  );
}
