...
import {
  getPlayerGoalBulkListSuccess,
  getPlayerGoalBulkListError,
} from './actions';
...
function* getPlayerGoalBulkList(action) {
  try {
    const goals = yield call(
      api.goalDefinitions.collaboratorGoalsBulk,
      action.definitionId,
      action.dates,
      action.teams,
    );
...
function* watchPlayerGoalBulkList() {
  yield takeLatest(types.GET_PLAYER_GOAL_BULK_LIST, getPlayerGoalBulkList);
}
