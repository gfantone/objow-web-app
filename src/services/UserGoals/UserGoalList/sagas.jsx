...
function* getUserGoalList(action) {
    try {
        let {data: goals} = yield call(api.userGoals.list, action.inProgress, action.team, action.category, action.date)
...
function* watchUserGoalList() {
    yield takeEvery(types.GET_USER_GOAL_LIST, getUserGoalList)
}
