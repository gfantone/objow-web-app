...
function* getGoalDefinitions(action) {
    try {
        ...
        if (action.includeData) {
            ...
            const pastPointList = yield all(definitions.map(definition => call(api.goalDefinitions.pastPoints, definition.id)));
            ...
            definitions.map(definition => {
                ...
                definition.pastPoints = pastPointList[index].data
            })
        }
...
function* getGoalDefinitionsByCollaborator(action) {
    try {
        ...
        if (action.detail) {
            ...
            const pastPointList = yield all(definitions.map(definition => call(api.goalDefinitions.pastPoints, definition.id, null, action.collaboratorId)));
...
function* getGoalDefinitionsByTeam(action) {
    try {
        ...
        if (action.detail) {
            ...
            const pastPointList = yield all(definitions.map(definition => call(api.goalDefinitions.pastPoints, definition.id, action.teamId, null)));
