...
function* getCollaboratorGoalSummaryList(action) {
    try {
        var {data: goals} = yield call(api.collaborators.collaboratorGoals, action.id, action.current, action.category, action.year, action.start, action.end, action.name, action.definition)
