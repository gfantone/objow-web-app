...
class AdminTeamUpdate extends MainLayoutComponent {
  ...
  handleSubmit(m) {
    ...
    const newCollaborators = this.collaborators.filter((c) =>
      model.collaborators.includes(c.id)
    );
    const oldCollaborators = this.id
  ...
  componentDidMount() {
    ...
    this.props.handleSubHeader(
      <AppBarSubTitle title="Modification d'une équipe" />
    );
  ...
  renderCollaboratorSelector(index, id = null) {
    ...
    collaborators = collaborators.filter(
      (collaborator) => !ids.includes(collaborator.id)
    );
                ...
                <Grid item>
                  {!hasCollaborators && (
                    <ProgressButton
                      ...
                      color='secondary'
                      text={'Supprimer'}
                      loading={teamRemovingLoading}
                      centered
                      onClick={this.handleRemoveTeam.bind(this)}
                    />
                  )}
                </Grid>
                <Grid item>
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withStyles(styles)(injectIntl(AdminTeamUpdate)));
