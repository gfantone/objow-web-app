...
class Filters extends Component {
  ...
  renderData() {
    ...
    if (selectedTeam && !selectedTeam.collaborators) {
      selectedTeam.collaborators = [];
      api.teams
        .collaborators(selectedTeam.id, { simpleCollaborators: true })
        .then((response) => {
          selectedTeam.collaborators = response.data;
          this.setState({
            ...this.state,
          });
        });
    ...
    const deleteTeam = selectedCollaborator
      ? {}
      : { onDelete: this.handleDeleteTeam };
    return (
      <div className={this.props.classes.filterChips}>
        {selectedTeam && _.includes(['A', 'M', 'S'], account.role.code) && (
          <Chip
            size="small"
            label={
        ...
        {!selectedTeam && (
          <Chip
            size="small"
            label={intl.formatMessage({ id: 'filter.global_chip' })}
            variant="outlined"
            className={this.props.classes.filterChip}
          />
        )}
        {selectedCollaborator &&
          _.includes(['M', 'A', 'S'], account.role.code) && (
            <Chip
              size="small"
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withStyles(styles)(injectIntl(Filters)));
