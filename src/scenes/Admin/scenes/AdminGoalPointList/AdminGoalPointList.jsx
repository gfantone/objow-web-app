...
class AdminGoalPointList extends MainLayoutComponent {
  ...
  refresh(team, collaborator, type) {
    ...
    var url = `/admin/periods/${periodId}/goal-levels?type=${
      type || this.state.type
    }`;

    if (team && !collaborator) url += `&team=${team}`;
    if (team && collaborator)
      url += `&team=${team}&collaborator=${collaborator}`;
  ...
  loadData = (force) => {
    ...
    if (
      ...
      if (team || collaborator) {
        ...
        this.props.goalDefinitionListActions.getGoalDefinitionList(
          periodId,
          true,
          true,
          true
  ...
  onSelectRepartitionMode = (repartitionId, mode) => {
    ...
    const newRepartitions = this.state.newRepartitions.find(
      (repartition) => repartition.id === repartitionId
    )
      ? this.state.newRepartitions.map((repartition) =>
          repartition.id === repartitionId
            ? Object.assign({}, repartition, newRepartition)
            : repartition
        )
  ...
  onRepartitionChange = (changes, totalPoints) => {
    ...
    changes.forEach((change) => {
      if (
        change.cell.type === 'repartitionPoints' ||
        change.cell.type === 'importance_percent'
      ) {
        let newRepartition = {
          id: change.cell.id,
        };
        if (change.cell.type === 'repartitionPoints') {
          ...
          newRepartition.importance_percent =
            (change.value / totalPoints) * 100;
        }
        ...
        newRepartitions = newRepartitions.find(
          (repartition) => repartition.id === newRepartition.id
        )
          ? this.state.newRepartitions.map((repartition) =>
              repartition.id === newRepartition.id
                ? Object.assign({}, repartition, newRepartition)
                : repartition
            )
          : [...this.state.newRepartitions, newRepartition];
  ...
  onSubmitRepartitions = () => {
    const { loading, success, error } =
      this.props.goalDefinitionPointRepartitionListUpdateActions.updateGoalDefinitionPointRepartitionList(
        this.state.newRepartitions.map((repartition) => {
          let result = {};
  ...
  getDisabledLinesFromDefinitions = (
    definitions,
    pointRepartitions,
    repartitionModes
  ) => {
    ...
    definitions.forEach((definition) => {
      if (this.team || this.collaborator) {
        ...
        const newRepartition = this.state.newRepartitions.find(
          (newRepartition) => newRepartition.id === repartition.id
        );

        const mode = repartition
          ? repartitionModes.find((mode) =>
              _.get(newRepartition, 'mode')
                ? mode.id === newRepartition.mode
                : mode.id === repartition.mode
            )
          : { code: 'G' };

        disabledLines[definition.id] = !(
          (this.team && !this.collaborator && mode.code === 'T') ||
          (this.collaborator && mode.code === 'I') ||
          (!this.collaborator && !this.team && mode.code === 'G')
        );
      }
    });
  ...
  addRepartitionPointsToDefinitions = (definitions) => {
    ...
    return definitions.map((definition) => {
      ...
      const customRepartitions = context
        ? this.getRepartitionsByContextAndDefinition(definition, context)
        : [];
  ...
  getRepartitionModesByCurrentRepartition = (
    currentRepartition,
    currentTeamRepartition
  ) => {
    ...
    if (this.team && !this.collaborator) {
      ...
      return repartitionModes.filter(
        (mode) =>
          (this.state.type === 'C' && mode.code === 'I') ||
          mode.id === _.get(currentTeamRepartition, 'mode')
      );
  ...
  getRepartitionsByContextAndDefinition = (definition, context) => {
    const { pointRepartitions } = this.props.goalDefinitionPointRepartitionList;
    return pointRepartitions.filter(
      (repartition) =>
        repartition.definition === definition.id && _.get(repartition, context)
    );
  };
  ...
  renderData() {
    ...
    const {
      usedCollaboratorPoints,
      currentCollaboratorPoints,
      usedTeamPoints,
      currentTeamPoints,
    } = definitions.reduce(
      (acc, definition) => {
      ...
      {
        usedCollaboratorPoints: 0,
        currentCollaboratorPoints: 0,
        usedTeamPoints: 0,
        currentTeamPoints: 0,
      }
    );
    ...
    const usableCollaboratorGoalPoints = baseCollaboratorGoalPoints
      ? baseCollaboratorGoalPoints -
        usedCollaboratorPoints -
        currentCollaboratorPoints
      : 0;
    const usableTeamGoalPoints = baseTeamGoalPoints
      ? baseTeamGoalPoints - usedTeamPoints - currentTeamPoints
      : 0;
    const { pointRepartitions } = this.props.goalDefinitionPointRepartitionList;
    const { modes: repartitionModes } =
      this.props.goalDefinitionPointRepartitionModeList;
    const filteredDefinitions = definitions.filter(
      (definition) => definition.type.code === this.state.type
    );

    const maxPoints =
      this.state.type === 'T' ? baseTeamGoalPoints : baseCollaboratorGoalPoints;
    const currentTeam = teams.find(
      (team) => this.team && team.id === parseInt(this.team)
    );
    const currentCollaborator =
      currentTeam &&
      this.collaborator &&
      _.get(currentTeam, 'collaborators', []).find(
        (collaborator) => collaborator.id === parseInt(this.collaborator)
      );

    const disabledLines = this.getDisabledLinesFromDefinitions(
      definitions,
      pointRepartitions,
      repartitionModes
    );
    ...
    var columns = _.compact([
      ...
      {
        name: 'name',
        label: intl.formatMessage({ id: 'admin.points.columns.name' }),
      },
      // { name: 'type.description', label: 'Objectif' },
      {
        name: 'usedPoints',
        label: intl.formatMessage({ id: 'admin.points.columns.used_points' }),
      },

      {
        name: 'currentPoints',
        label: intl.formatMessage({
          id: 'admin.points.columns.current_points',
        }),
      },
      ...
      {
        name: 'category',
        label: intl.formatMessage({ id: 'admin.points.columns.category' }),
        options: {
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
      ...
      !this.team || (this.state.type !== 'T' && this.team && !this.collaborator)
        ? {
            ...
            label: globalMode
              ? intl.formatMessage({ id: 'admin.points.columns.custom_team' })
              : intl.formatMessage({
                  id: 'admin.points.columns.custom_individual',
                }),
            options: {
              filter: false,
              customBodyRender: (value, tableMeta, updateValue) => {
                ...
                value.forEach((repartition, i) => {
                  if (repartition.mode === baseRepartitionMode.id) {
                    const newEntity = currentTeam
                      ? currentTeam.collaborators?.find(
                          (collaborator) =>
                            collaborator.id === repartition.collaborator
                        )
                ...
                if (entities.length > 0) {
                  ...
                  const toolTipText = entities.reduce(
                    (acc, entity) => (
                      <div>
                        {acc}
                        {`${
    ...
    const options = {
      ...
      onRowClick: (colData, cellMeta) => {
        ...
        const mode = repartitionModes.find((mode) =>
          newRepartition && _.get(newRepartition, 'mode')
            ? mode.id === newRepartition.mode
            : mode.id === repartition.mode
        );
    ...
    const percentByDefinition = (definition) =>
      Number(
        (
          ((definition.usedPoints + definition.currentPoints) / maxPoints) *
          100
        ).toFixed(2)
      );
    const totalPointsPercent = Number(
      filteredDefinitions
        .reduce((acc, definition) => acc + percentByDefinition(definition), 0)
        .toFixed(2)
    );
            ...
            <Grid container spacing={4}>
                  ...
                  <Grid container spacing={1}>
                    <Grid item>
                      <BigText isContrast>
                        ...
                        {currentTeam || currentCollaborator
                          ? `pour ${
                              _.get(currentCollaborator, 'fullname') ||
                              _.get(currentTeam, 'name')
                            }`
                          : ''}
                      </BigText>
                    </Grid>
                            ...
                            <React.Fragment>
                              <React.Fragment>
                                <Grid item>
                                  ...
                                  >
                                    ...
                                    >
                                      <DefaultText>
                                        {usableCollaboratorGoalPoints.toLocaleString()}
                                      </DefaultText>
                                    </Grid>
                                    <Grid
                                      item
                                      className={classes.headerPointsLabel}
                                    >
                                      <DefaultText>
                                        points / joueur disponibles
                                      </DefaultText>
                                    </Grid>
                                  </Grid>
                                </Grid>
                                <Grid item>
                                  ...
                                  >
                                    <Grid
                                      item
                                      className={`${classes.headerPoints} ${classes.usedPoints}`}
                                    >
                                      <DefaultText>
                                        {usedCollaboratorPoints.toLocaleString()}
                                      </DefaultText>
                                    </Grid>
                                    <Grid
                                      item
                              ...
                              <Grid item>
                                ...
                                >
                                  ...
                                  >
                                    <DefaultText>
                                      {currentCollaboratorPoints.toLocaleString()}
                                    </DefaultText>
                                  </Grid>
                                  <Grid
                                    item
                            ...
                            <React.Fragment>
                              <Grid item>
                                ...
                                >
                                  ...
                                  >
                                    <DefaultText>
                                      {usableTeamGoalPoints.toLocaleString()}
                                    </DefaultText>
                                  </Grid>
                                  <Grid item>
                                    <DefaultText>
                                      pts / équipe disponible
                                    </DefaultText>
                                  </Grid>
                                </Grid>
                              </Grid>
                              <Grid item>
                                ...
                                >
                                  ...
                                  >
                                    <DefaultText>
                                      {usedTeamPoints.toLocaleString()}
                                    </DefaultText>
                                  </Grid>
                                  <Grid item>
                                    <DefaultText>
                                      pts / équipe déjà mis en jeu
                                    </DefaultText>
                                  </Grid>
                                </Grid>
                              </Grid>
                              <Grid item>
                                ...
                                >
                                  ...
                                  >
                                    <DefaultText>
                                      {currentTeamPoints.toLocaleString()}
                                    </DefaultText>
                                  </Grid>
                                  <Grid item>
                                    <DefaultText>
                                      pts / équipe en cours de jeu
                                    </DefaultText>
                                  </Grid>
                                </Grid>
                              </Grid>
              ...
              <Grid item sm={12}>
                <Grid container spacing={1} direction='column'>
                  ...
                  <Grid item>
                    <DataTable
                      data={this.addRepartitionPointsToDefinitions(
                        filteredDefinitions
                      )}
                      columns={columns}
                      options={options}
          ...
          {displayRepartition && (
            <Grid item sm={4}>
              <Grid container spacing={1}>
                <Grid item xs={12}>
                  <BigText isContrast>
                    Matrice de répartition des points
                  </BigText>
                </Grid>
                <Grid item>
                  <Card>
                      ...
                      <Grid item>
                        ...
                        >
                          <Grid item className={`${classes.headerPoints}`}>
                            <DefaultText>
                              {baseGoalPoints.toLocaleString()}
                            </DefaultText>
                          </Grid>
                          <Grid item>
                            <DefaultText>
                              Max de points gagnables /{' '}
                              {this.state.type === 'T'
                                ? `équipe ${
                                    currentTeam
                                      ? `(${_.get(
                                          currentTeam,
                                          'collaborator_ids.length'
                                        )} joueurs)`
                                      : ''
                                  }`
                                : 'joueur'}
                          ...
                          <Grid container spacing={2}>
                              ...
                              <ReactDataSheet
                                data={[
                                  [
                                    ...
                                    {
                                      value: 'Points Alloués',
                                      readOnly: true,
                                      className: 'headerCell',
                                    },
                                    {
                                      value: 'Mode de répartition',
                                      readOnly: true,
                                      className: 'headerCell',
                                    },
                                  ],
                                  ...filteredDefinitions.map((definition) => {
                                    ...
                                    const repartition =
                                      pointRepartitions.filter(
                                        (pointRepartition) =>
                                          pointRepartition.definition ===
                                            definition.id &&
                                          ((this.team &&
                                            !this.collaborator &&
                                            pointRepartition.team ===
                                              parseInt(this.team)) ||
                                            (this.collaborator &&
                                              pointRepartition.collaborator ===
                                                parseInt(this.collaborator)))
                                      )[0];

                                    const teamRepartition =
                                      pointRepartitions.filter(
                                        (pointRepartition) =>
                                          pointRepartition.definition ===
                                            definition.id &&
                                    ...
                                    const newRepartition =
                                      this.state.newRepartitions.find(
                                        (newRepartition) =>
                                          newRepartition.id === repartition.id
                                      );
                                    ...
                                    let repartitionPoints =
                                      definition.currentPoints +
                                      definition.usedPoints;
                                    let importance_percent =
                                      (repartitionPoints / maxPoints) * 100;

                                    if (mode.code !== 'G') {
                                      importance_percent =
                                        repartition && repartition.points;
                                      ...
                                      if (
                                        ...
                                        importance_percent =
                                          (individualPoints / maxPoints) * 100;
                                      }
                                      ...
                                      if (
                                        ...
                                        importance_percent = _.get(
                                          pointRepartitions.find(
                                            (pointRepartition) =>
                                              pointRepartition.definition ===
                                                definition.id &&
                                              currentCollaborator &&
                                              currentCollaborator.team.id ===
                                                pointRepartition.team
                                          ),
                                          'points',
                                          0
                                      ...
                                      importance_percent =
                                        ...
                                        importance_percent;
                                      repartitionPoints =
                                        (maxPoints * importance_percent) / 100;
                                    ...
                                    const repartitionReadonly =
                                      ...
                                      (mode.code === 'T' &&
                                        (!this.team || this.collaborator)) ||
                                      (mode.code === 'I' && !this.collaborator);

                                    if (Number(importance_percent)) {
                                      totalImportancePercent +=
                                        Number(importance_percent);
                                    }
                                    if (Number(repartitionPoints)) {
                                      totalAvailable +=
                                        Number(repartitionPoints);
                                    }
                                    ...
                                    return [
                                      ...
                                      {
                                        value: Number(
                                          Number(importance_percent).toFixed(2)
                                        ).toLocaleString(),
                                      ...
                                      {
                                        value:
                                          repartitionPoints.toLocaleString(),
                                        readOnly: repartitionReadonly,
                                      ...
                                      },
                                      {
                                        ...
                                        type: 'select',
                                        choices:
                                          this.getRepartitionModesByCurrentRepartition(
                                            repartition,
                                            teamRepartition
                                          ),
                                        id: repartition.id,
                                  ...
                                  [
                                    ...
                                    {
                                      value: Number(
                                        totalImportancePercent.toFixed(2)
                                      ).toLocaleString(),
                                      readOnly: true,
                                      className: 'pointsCell',
                                    },
                                ...
                                onCellsChanged={(changes) =>
                                  this.onRepartitionChange(changes, maxPoints)
                                }
                                valueRenderer={(cell) => cell.value}
                              />
                            </Grid>
                            <Grid item container spacing={2}>
                              {Number(totalImportancePercent) &&
                                totalImportancePercent > 100 && (
                                  <Grid item justify='center'>
                                    <ErrorText
                                      className={classes.error}
                                      align='center'
                                    >{`Le total de points ne peut pas dépasser le total alloué pour les objectifs (${
                                      this.state.type === 'T'
                                        ? baseTeamGoalPoints.toLocaleString()
                                        : baseCollaboratorGoalPoints.toLocaleString()
                                    } points)`}</ErrorText>
                                  </Grid>
                                )}
                              {!allRepartitionsValid && (
                                <Grid item justify='center'>
                                  <ErrorText
                                    className={classes.error}
                                    align='center'
                                  >{`Les points alloués de chaque objectif ne peuvent pas être inférieurs aux points déjà mis en jeu`}</ErrorText>
                                </Grid>
                              )}
                            </Grid>
                            <Grid item xs={12} alignItems='center'>
                              <ProgressButton
                                ...
                                loading={this.state.repartitionLoading}
                                centered
                              />
                            </Grid>
                          </Grid>
  ...
  render() {
    ...
    const { success, hasError } =
      this.props.goalDefinitionPointRepartitionListUpdate;
    ...
    const { definitions, loading: goalDefinitionListLoading } =
    ...
    const {
      ...
      loading: goalDefinitionPointRepartitionModeLoading,
    } = this.props.goalDefinitionPointRepartitionModeList;
    const loading =
      configfListLoading ||
      goalDefinitionListLoading ||
      goalDefinitionPointRepartitionLoading ||
      goalDefinitionPointRepartitionModeLoading;

    if (success) {
      this.props.goalDefinitionPointRepartitionListUpdateActions.updateGoalDefinitionPointRepartitionListClear();
      toast.success(
...
const mapDispatchToProps = (dispatch) => ({
  ...
  goalDefinitionLevelCollaboratorPointsActions: bindActionCreators(
    goalDefinitionLevelCollaboratorPointsActions,
    dispatch
  ),
  goalDefinitionLevelTeamPointsActions: bindActionCreators(
    goalDefinitionLevelTeamPointsActions,
    dispatch
  ),
  ...
  goalDefinitionPointRepartitionListUpdateActions: bindActionCreators(
    goalDefinitionPointRepartitionListUpdateActions,
    dispatch
  ),
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withStyles(styles)(withRouter(injectIntl(AdminGoalPointList))));
