...
const StatsData = ({collaborator, definition, display, period, team, ...props}) => {
    const {goals: collaboratorGoalSummaryListGoals, loading: collaboratorGoalSummaryListLoading} = props.collaboratorGoalSummaryList
    ...
    const {goals: teamCollaboratorGoalListGoals, loading: teamCollaboratorGoalListLoading} = props.teamCollaboratorGoalList
    const {goals: teamGoalSummaryListGoals, loading: teamGoalSummaryListLoading} = props.teamGoalSummaryList
    ...
    const loading = collaboratorGoalSummaryListLoading || teamCollaboratorGoalListLoading || teamGoalSummaryListLoading
    ...
    useEffect(() => {
        const definitionObjects = definition && definitions ? definitions.filter(x => x.id === definition) : null
        const definitionObject = definitionObjects && definitionObjects.length === 1 ? definitionObjects[0] : null

        if (collaborator && definitionObject && definitionObject.typeCode === 'C') {
            props.collaboratorGoalSummaryListActions.getCollaboratorGoalSummaryListByDefinitionAndCollaborator(definition, collaborator)
        ...
        } else if (collaborator && definitionObject && definitionObject.typeCode === 'T') {
            props.teamGoalSummaryListActions.getTeamGoalSummaryListByDefinitionAndCollaborator(definition, collaborator)
            ...
            props.teamCollaboratorGoalListActions.getTeamCollaboratorGoalListByDefinitionAndTeam(definition, team)
            props.collaboratorGoalSummaryListActions.clearCollaboratorGoalSummaryList()
            props.teamGoalSummaryListActions.clearTeamGoalSummaryList()
        } else if (team && definitionObject && definitionObject.typeCode === 'T') {
            props.teamGoalSummaryListActions.getTeamGoalSummaryListByDefinitionAndTeam(definition, team)
            props.collaboratorGoalSummaryListActions.clearCollaboratorGoalSummaryList()
            props.teamCollaboratorGoalListActions.clearTeamCollaboratorGoalList()
        } else {
            props.collaboratorGoalSummaryListActions.clearCollaboratorGoalSummaryList()
...
const mapStateToProps = ({collaboratorGoalSummaryList, currentPeriodDetail, goalDefinitionList, previousPeriodList, teamCollaboratorGoalList, teamGoalSummaryList}) => ({
