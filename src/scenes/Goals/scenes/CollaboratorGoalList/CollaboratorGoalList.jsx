...
class CollaboratorGoalList extends MainLayoutComponent {
  ...
  loadData(props) {
    ...
    if (
      ...
      if (onlyTeam === true) {
        this.props.teamGoalListSummaryActions.getTeamGoalSummaryListByCollaborator(
          id,
          this.current,
          this.category,
          this.year,
      ...
      <div>
        <Grid container spacing={3}>
          {goals.map((goal) => {
            const url =
              goal.type == 'C'
                ? `/goals/detail/collaborator/${goal.id}`
  ...
  render() {
    ...
    return (
      <div>
        {_.get(account, 'role.code') !== 'C' && !_.get(account, 'isJtiEnv') && (
          <GoalCollaboratorFilter
...
const mapDispatchToProps = (dispatch) => ({
  ...
  teamGoalListSummaryActions: bindActionCreators(
    teamGoalListSummaryActions,
    dispatch
  ),
});
