...
const SubHeader = ({ activateRank, onChange, ...props }) => {
  ...
  var { challenge, loading: collaboratorChallengeDetailLoading } =
    props.collaboratorChallengeDetail;
  const { loading: collaboratorChallengeRankListLoading } =
    props.collaboratorChallengeRankList;
  const { loading: collaboratorChallengeGoalListLoading } =
    props.collaboratorChallengeGoalList;
  const loading =
    collaboratorChallengeDetailLoading ||
    collaboratorChallengeGoalListLoading ||
    rankLoading;

  const { account } = props.accountDetail;
  const isJti = account.isJtiEnv;

  if (!collaboratorChallengeGoalListLoading && rankLoading) {
  ...
  const fetchWonAwards = (challenge) => {
    let query;
  ...
  const fetchCurrentRank = (challenge) => {
    let query;
    if (challenge.typeCode === 'CC') {
        ...
        <div style={{ opacity: scroll > 0 ? 0 : 1 }} ref={baseRef}>
          <div className={classes.challengeContainer}>
            {isJti ? (
              <ChallengeSimpleJti
                challenge={challenge}
                fetchWonAwards={() => fetchWonAwards(challenge)}
                fetchCurrentRank={() => fetchCurrentRank(challenge)}
                fetchGoalPoints={() =>
                  api.challenges.goal_points(challenge.sourceId, {
                    collaborator_id: challenge.collaboratorId,
                  })
                }
              />
            ) : (
